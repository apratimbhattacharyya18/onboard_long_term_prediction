...
class Recurrent(Layer):
    ...
    def get_initial_state(self, inputs):
        ...
        if len(self.states) == 4:
            initial_state[2] = K.dropout(K.ones_like(K.sum(inputs, axis=(1))),0.35);
            initial_state[3] = K.dropout(K.ones_like(initial_state[1]),0.35);
...
class dLSTM(Recurrent):
    ...
    def __init__(self, units,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(dLSTM, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.recurrent_activation = activations.get(recurrent_activation)
        self.use_bias = use_bias
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            z += K.dot(h_tm1 * rec_dp_mask, self.recurrent_kernel)
            if self.use_bias:
            ...
            if self.implementation == 0:
                ...
                x_i = K.dot(inputs * dp_mask, self.kernel_i) + self.bias_i
                x_f = K.dot(inputs * dp_mask, self.kernel_f) + self.bias_f
                x_c = K.dot(inputs * dp_mask, self.kernel_c) + self.bias_c
                x_o = K.dot(inputs * dp_mask, self.kernel_o) + self.bias_o
            else:
                raise ValueError('Unknown `implementation` mode.')

            i = self.recurrent_activation(x_i + K.dot(h_tm1 * rec_dp_mask,
                                                      self.recurrent_kernel_i))
            f = self.recurrent_activation(x_f + K.dot(h_tm1 * rec_dp_mask,
                                                      self.recurrent_kernel_f))
            c = f * c_tm1 + i * self.activation(x_c + K.dot(h_tm1 * rec_dp_mask,
                                                            self.recurrent_kernel_c))
            o = self.recurrent_activation(x_o + K.dot(h_tm1 * rec_dp_mask,
                                                      self.recurrent_kernel_o))
        h = o * self.activation(c)
        #if 0 < self.dropout + self.recurrent_dropout:
        #    h._uses_learning_phase = True
        return h, [h, c, dp_mask, rec_dp_mask]
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(dLSTM, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
